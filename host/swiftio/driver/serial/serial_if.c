...
{
    if (serial_hdl->queue) {
		swifthal_os_mq_destory(serial_hdl->queue);
	...
	uint8_t * wbuffer, const uint16_t wlen)
{
	...
	return esp_device_if_transaction(serial_hdl->if_type,
...
/**
  ...
  * @retval None
  */
int serial_rx_handler(uint8_t if_num, uint8_t *rxbuff, uint16_t rx_len)
{
	interface_buffer_handle_t buf_handle = {0};
    ...
    swifthal_os_mq_send(serial_hdl->queue, &buf_handle, -1);

...
vhci_handle_t *ble_vhci_open(void (*hci_rx_callback)(struct vhci_handle_s *, void *), void *arg)
{
	...
	vhci_handle_t *vhci_hdl = malloc(sizeof(vhci_handle_t));
	if (vhci_hdl == NULL) {
		return NULL;
	}
	...
	vhci_hdl->queue = swifthal_os_mq_create(sizeof(interface_buffer_handle_t), TO_VHCI_INFT_QUEUE_SIZE);
	if (vhci_hdl->queue == NULL) {
		free(vhci_hdl);
		return NULL;
	}
...
void ble_vhci_close(vhci_handle_t *vhci_hdl)
{
	...
	if (vhci_hdl->queue) {
		swifthal_os_mq_destory(vhci_hdl->queue);
        vhci_hdl->queue = NULL;
	}
...
uint8_t *ble_vhci_read(const vhci_handle_t *vhci_hdl, uint16_t *rlen)
{
	...
	*rlen = 0;
	 ...
	 */
    int ret = swifthal_os_mq_recv(vhci_hdl->queue, &buf_handle, 0);
    if (ret != 0) {
...
int ble_vhci_write(const vhci_handle_t *vhci_hdl, uint8_t *wbuffer, const uint16_t wlen)
{
	...
	return esp_device_if_transaction(vhci_hdl->if_type, vhci_hdl->if_num, wbuffer, wlen);
}
...
int vhci_rx_handler(uint8_t if_num, uint8_t *rxbuff, uint16_t rx_len)
{
	...
	/* send to vhci queue */
    int ret = swifthal_os_mq_send(vhci_hdl->queue, &buf_handle, -1);
    if (ret != 0) {
